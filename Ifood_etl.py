...
#df  = base manipulada com criação de colunas e transformação de dados
...
df_senhas = pd.read_excel(r'G:\Meu Drive\#######.xlsx', sheet_name = 'Acessos')
df_acessos = pd.read_excel(r'G:\Meu Drive\\#######.xlsx', sheet_name = 'Lojas')
...
# foi verificado em uma das análises a presença de linhas duplicadas
df = df.drop_duplicates()


# ## Estruturação de faturamentos (Bruto, Líquido e Real)
# 

# O faturamento obtido na plataforma não é de acordo com o valor de repasse do restaurante.                             
# Diversas taxas só são aplicadas nos últimos cálculos de repasse da plataforma, impossibilitando a auditoria de valores.          
# Para identificar as taxas, criamos variáveis e validamos as relações durante aproximadamente um mês com assertividade média de 98% no período.
# 
...
# Base de cálculo para faturamentos
...
# ## Classificando os novos dados de finalidade
# 
# 1. FATURAMENTO BRUTO : valor financeiro obtido com todas as transações da plataforma, independente de investimentos, taxas ou cancelamentos
# 2. FATURAMENTO LÍQUIDO : valor financeiro obtido após a consideração de investimentos e cancelamentos
# 3. FATURAMENTO REAL : valor financeiro final que deveria ser efetuado no repasse com todas as considerações da plataforma
# 4. TAXA EFETIVA : taxa percentual que relaciona o investimento para operacionar na plataforma
# 5. MARGEM : taxa percentual de "lucratividade" da operação na plataforma
# 6. VALOR DE TAXA DE TRANSAÇÃO:  taxa fixa cobrada para operação na plataforma
# 7. VALOR DE TAXA DE 1 SEMANA: taxa fica cobrada para repasse do "lucro" na plataforma
# 8. NÚMERO DE PEDIDOS
# 9. NÚMERO DE CANCELAMENTOS
# 10. VALOR BRUTO CANCELADO NA PLATAFORMA : valor perdido em cancelamentos, independente se realizado por clientes ou restaurantes
# 11. VALOR LÍQUIDO CANCELADO NA PLATAFORMA : valor real perdido em cancelamentos com considerações de responsável pelo cancelamento
# 12. CANCELAMENTO(%)
# 13. CANCELAMENTO PELO CLIENTE (%)
# 14. CANCELAMENTO PELO RESTAURANTE (%)
# 15. TAXA DE CANCELAMENTO
# 16. TICKET MÉDIO : média de valor cobrado por produtos na plataforam
# 17. TICKET MÉDIO REAL : média de valor cobrado por produtos na plataforam sem considerações de taxas e investimentos

# In[71]:


# Substituir colunas numeric
df1_financeiro[['FATURAMENTO BRUTO','FATURAMENTO LÍQUIDO','FATURAMENTO REAL','TAXA EFETIVA','INCENTIVO IFOOD','INCENTIVO LOJA','VALOR DE TAXA DE TRANSAÇÃO','VALOR DE TAXA DE 1 SEMANA','NÚMERO DE PEDIDOS','NÚMERO DE CANCELAMENTOS','VALOR BRUTO CANCELADO NA PLATAFORMA','VALOR LÍQUIDO CANCELADO NA PLATAFORMA','TAXA DE ENTREGA','CANCELAMENTO(%)','CANCELAMENTO PELO CLIENTE','CANCELAMENTO PELO CLIENTE (%)','CANCELAMENTO PELO RESTAURANTE','CANCELAMENTO PELO RESTAURANTE (%)','TICKET MÉDIO','TICKET MÉDIO REAL','INCENTIVO (%)','TAXA EFETIVA','MARGEM','TAXA DE CANCELAMENTO','NPS MÉDIO','NÚMERO DE COMENTÁRIOS','NÚMERO DE AVALIAÇÕES']].apply(pd.to_numeric)
